...
void APPLE_SETUP(const int width, const int height, int* X_apple, int* Y_apple, int* array_X, int* array_Y, int* length) { // установка координат "яблока"
	do // цикл ставит координаты яблока случанйм образом - но чтобы они не совпадали со "змейкой"
	{
		*X_apple = rand() % (width - 2) + 1; // установка координат "яблока" по оси абсцисс
		*Y_apple = rand() % (height - 2) + 1; // установка координат "яблока" по оси ординат
	} while (*X_apple != array_X[*length - 1] && *Y_apple != array_Y[*length - 1]); // проверка на совпадение с "змейкой"
}

void DRAWING_FRAME(const int* height, const int* width, HANDLE h) { // отрисовка рамки поля
...
void INFO_PANEL(COORD& info_panel, HANDLE h, int max_length, int length, const int width) { // отрисовка информационной панели
...
void UPDATE_LENGTH(COORD info_panel, HANDLE h, int length, int width) { // обновление длины "змейки" в информационной панели
	info_panel.X = width + 5;
	info_panel.Y = 2;
	SetConsoleCursorPosition(h, info_panel); 
	SetConsoleOutputCP(1251);
...
void MAIN_CYCLE(const int max_length, char snake, char apple, char head, int sleep_time, int dx, int dy, int* array_X, int* array_Y, int* length, const int width, const int height, HANDLE h, int* X_apple, int* Y_apple, COORD c, COORD info_panel) { // основной цикл игры
